...
def printResults():
    ...
    print('Average Change: $', round(abs(total_revenue_change) / (len(rowsArray) - 1), 2))
...
with open(csvpath, newline='') as csvfile:
    ...
    for i in range(1, len(rowsArray)):
        ...
        if max_length_not_reached:
            total_revenue_change = int(rowsArray[i+1][1]) - int(rowsArray[i ][1])
            ...
            if (greatest_decrease < int(rowsArray[i][1]) - int(rowsArray[i + 1][1])):
                greatest_decrease = int(rowsArray[i + 3][1]) - int(rowsArray[i][1])
